...
// 멤버 메서드 : 클래스에 소속되어 클래스 객체에 종속된 메서드입니다.
class Animal {
	private String name;
	private int age;

	// 멤버 메서드는 특별한 경우를 제외하곤 static을 사용하지 않습니다
	// 현재 메서드를 호출하는 호출객체(a1, a2, 등)를 별도의 매개변수로 받지 않아도 전달어져 있습니다.
	public void prn() {
		// 호출 객체는 this로 전달됩니다. 안보입니다. 모르게 전달됩니다
		System.out.println("저의 이름은 " + this.name + ", 나이는 " + this.age + "살 입니다");
	}

	// 멤버 메서드는 같은ㅌ 클래스내의 private 멤버변수에 자유롭게 접근이 가능합니다
...
//	}
	...
	//	그 목적만 있다면 메서드명이 반드시 위와 같지 않아도 무방합니다.
	// 그러나 getter와 setter는 개발자가 호출하여 사용하기도 하지만
	// 시스템이 또는 컴파일러가 사용하기도 때문에 약속된 이름을(get변수이름,set변수이름)을 사용하는것이 일반적입니다.
...
//				  같은 패키지내에서 public처럼 자유롭게 접근 가능합니다. 외부패키지에서는 private이나 마찬가지
...
//		System.out.println("저의 이름은 " + a1.name + ", 나이는 " + a1.age + "살 입니다");
		System.out.println("저의 이름은 " + a1.getName() + ", 나이는 " + a1.getName() + "살 입니다");
	}
